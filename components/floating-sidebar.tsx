            ...
            {navigationItems.map((item, index) => (
              ...
              >
                <Button
                  ...
                  className={`relative w-full h-10 rounded-lg transition-all duration-300 border-0 justify-start px-3 ${
                ...
                >
                  <div className="flex items-center gap-3 w-full">
                    ...
                    <motion.div
                      animate={{
                        ...
                        rotate: isHovered && activeSection === item.id ? [0, -5, 5, 0] : 0,
                      }}
                      transition={{
                        duration: 0.3,
                        type: "tween",
                  ...
                  {activeSection === item.id && (
                    <motion.div
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full shadow-md shadow-cyan-400/40"
                    />
                  )}
                  ...
                  <motion.div
                    ...
                    className={`absolute inset-0 rounded-xl bg-gradient-to-r ${
                      activeSection === item.id ? "from-cyan-400/20 to-blue-500/20" : "from-white/10 to-white/5"
                    }`}
                  />
                </Button>
                ...
                <motion.div
                  initial={{ scale: 0, opacity: 0 }}
                  animate={{
                    scale: isHovered && activeSection === item.id ? [0, 1.5, 0] : 0,
                    opacity: isHovered && activeSection === item.id ? [0, 0.3, 0] : 0,
                  }}
                  transition={{
                    duration: 1.5,
                    repeat: isHovered && activeSection === item.id ? Number.POSITIVE_INFINITY : 0,
                    repeatDelay: 0.5,
                  }}
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.nav
              ...
              transition={{ duration: 0.4, type: "tween", stiffness: 300, damping: 30 }}
            ...
            >
                ...
                {navigationItems.map((item, index) => (
                  ...
                  >
                    <Button
                      variant="ghost"
                      onClick={() => scrollToSection(item.id)}
                      className={`w-full h-12 rounded-xl transition-all duration-300 justify-start px-4 ${
                        activeSection === item.id
                          ? "bg-white/20 text-cyan-400 shadow-lg shadow-cyan-400/20"
                          : "bg-transparent hover:bg-white/10 text-gray-400 hover:text-white"
                      }`}
                    >
                      ...
                      {activeSection === item.id && (
                        <motion.div
                          ...
                          exit={{ scale: 0, opacity: 0 }}
                          transition={{ duration: 0.3, type: "tween", stiffness: 300 }}
                          className="w-2 h-2 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full shadow-md shadow-cyan-400/40"
                        />
                      )}
                    </Button>
                  </motion.div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-gray-800">
                <p className="text-sm text-gray-500 text-center">Alex Johnson Portfolio</p>
              </div>
            </motion.nav>
          </>
        )}
              ...
              >
                ...
                >
                  <item.icon
                    className={`h-6 w-6 transition-colors duration-300 ${
                      activeSection === item.id ? "text-cyan-400" : "text-gray-400 group-hover:text-white"
                    }`}
                  />
                </motion.div>
                ...
                {activeSection === item.id && (
                  <motion.div
                    ...
                    exit={{ scale: 0, opacity: 0 }}
                    transition={{ duration: 0.3, type: "tween", stiffness: 300 }}
                    className="absolute -top-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full shadow-md shadow-cyan-400/40"
                  />
                )}
